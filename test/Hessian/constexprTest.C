// RUN: %cladclang %s -I%S/../../include /clang:-std=c++14 -oconstexprTest.out 2>&1 | %filecheck %s
// RUN: ./constexprTest.out | %filecheck_exec %s
// RUN: %cladclang -Xclang -plugin-arg-clad -Xclang -enable-tbr %s -I%S/../../include /clang:-std=c++14 -oconstexprTest.out
// RUN: ./constexprTest.out | %filecheck_exec %s
