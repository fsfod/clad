// RUN: %cladnumdiffclang /clang:-std=c++17 -Wno-writable-strings %s  -I%S/../../include -oFunctionCalls.out 2>&1 | %filecheck %s
// RUN: ./FunctionCalls.out | %filecheck_exec %s
// RUN: %cladnumdiffclang -Xclang -plugin-arg-clad -Xclang -enable-tbr /clang:-std=c++17 -Wno-writable-strings %s  -I%S/../../include -oFunctionCalls.out
// RUN: ./FunctionCalls.out | %filecheck_exec %s
